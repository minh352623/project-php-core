...
if (!empty($body['id'])) {
    ...
    $galleryDetailArr = getRaw("SELECT 	* FROM portfolio_images WHERE portfolio_id=$portfoliosId");
    $galleryData = [];
    $galleryIdArr  = [];

    if (!empty($galleryDetailArr)) {
        foreach ($galleryDetailArr as $key => $item) {
            $galleryData[] = $item['image'];
            $galleryIdArr[] = $item['id'];
        }
    }
    // echo '<pre>';
    // print_r($galleryIdArr);
    // echo '</pre>';
    ...
    if (empty($portfoliosDetail)) {
        ...
        // setFlashData('portfoliosDetail', $userDetail);
...
if (isPost()) {
    ...
    if (empty($errors)) {
        ...
        //kiểm tra data ảnh củ với mới có khác nhau không
        if (count($galleryArr) > count($galleryData)) {
            //insert những ảnh còn thiếu
            //update những ảnh thay đổi
            // $diff = array_diff($galleryArr, $galleryData);
            //nếu 2 mảng có dữ liệu khác nhau nó sẽ trả về mảng với phần tử khác nhau
            //nếu lúc đầu có 2 item mà lúc cập nhật xóa đi 1 item thì hàm này sẽ không biết dc có sự thay đổi
            //=> khắc phục
            if (!empty($galleryData)) {
                ...
                foreach ($galleryData as $key => $item) {
                    ...
                    $updateStatus = update('portfolio_images', $dataImages, $condition);
                }
            } else {
                $key = -1;
            }
            ...
            for ($i = $key + 1; $i < count($galleryArr); $i++) {
                ...
                //insert thư viện ảnh còn thiếu
                $insertStatus = insert("portfolio_images", $dataImages);
            }
            ...
            for ($i = $key + 1; $i < count($galleryData); $i++) {
                ...
                //xóa ảnh thừa
                // $condition = "image = '" . $galleryData[$i] . "'";
                $condition = "id=" . $galleryIdArr[$i];
...
$errors = getFlashData('errors') ? getFlashData('errors') : [];
$old = getFlashData('old');
if (empty($old) && !empty($portfoliosDetail)) {
    $old = $portfoliosDetail;

    $old['gallery'] = $galleryData;
}
$left = getFlashData('left');
